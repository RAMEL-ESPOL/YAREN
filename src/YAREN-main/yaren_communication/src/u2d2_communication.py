    ...
    for motor in list_motors:
        for id in motor.list_ids:
            ...
            if dxl_comm_result != COMM_SUCCESS:
                ...
                print("%s" % motor.packetHandler.getRxPacketError(dxl_error))
            ##################
            general_joint_position[id-1]=dxl_present_position
...
if __name__ == '__main__':
    ...
    neck_motor = XCseries_motor(usb_port,dxl_baud_rate,[3,4],portHandler,packetHandler,r,20,{3:[-1,1],4:[-1,1]},{3:[100,0,50],4:[100,0,50]})

    shoulder_right_motor = XCseries_motor(usb_port,dxl_baud_rate,[5],portHandler,packetHandler,r,20,{5:[-1,1]},{5:[100,0,50]})
    forearm_right_motor = XCseries_motor(usb_port,dxl_baud_rate,[6,7],portHandler,packetHandler,r,20,{6:[-1,1],7:[-1,1]},{6:[100,0,50],7:[100,0,50]})
    elbow_right_motor = XCseries_motor(usb_port,dxl_baud_rate,[8],portHandler,packetHandler,r,20,{8:[-1,1]},{8:[100,0,50]})

    shoulder_left_motor = XCseries_motor(usb_port,dxl_baud_rate,[9],portHandler,packetHandler,r,20,{9:[-1,1]},{9:[100,0,50]})
    forearm_left_motor = XCseries_motor(usb_port,dxl_baud_rate,[10,11],portHandler,packetHandler,r,20,{10:[-1,1],11:[-1,1]},{10:[100,0,50],11:[100,0,50]})
    elbow_left_motor = XCseries_motor(usb_port,dxl_baud_rate,[12],portHandler,packetHandler,r,20,{12:[-1,1]},{12:[100,0,50]})

    trunk_motor = MX_motor(usb_port,dxl_baud_rate,[1],portHandler,packetHandler,r,20,{1:[-1,1]},{1:[100,0,50]})
    hip_motor = MX_motor(usb_port,dxl_baud_rate,[2],portHandler,packetHandler,r,20,{2:[-1,1]},{2:[100,0,50]})
